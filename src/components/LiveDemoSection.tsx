...
const PhoneFrame = ({ children }: { children: React.ReactNode }) => (
  <div className="relative mx-auto" style={{ width: 280 }}>
    {/* Phone body */}
    <div className="relative bg-[var(--text-primary)] rounded-[44px] p-3 shadow-[0_50px_100px_rgba(0,0,0,0.4)]">
      {/* Notch */}
      <div className="absolute top-3 left-1/2 -translate-x-1/2 w-20 h-6 bg-[var(--text-primary)] rounded-b-xl z-10" />
      {/* Screen */}
      <div className="bg-[var(--bg-primary)] rounded-[36px] overflow-hidden border border-[var(--glass-border)]" style={{ height: 550 }}>
        {children}
      </div>
      {/* Home indicator */}
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-24 h-1 bg-[var(--bg-primary)]/20 rounded-full" />
    </div>
    {/* Side buttons */}
    <div className="absolute top-20 -left-1 w-1 h-10 bg-[var(--text-primary)]/40 rounded-l-sm" />
    <div className="absolute top-32 -left-1 w-1 h-8 bg-[var(--text-primary)]/40 rounded-l-sm" />
    <div className="absolute top-24 -right-1 w-1 h-14 bg-[var(--text-primary)]/40 rounded-r-sm" />
  </div>
);

// Browser frame for web apps
const BrowserFrame = ({ url, children }: { url: string; children: React.ReactNode }) => (
  <div className="relative rounded-[16px] overflow-hidden border border-[var(--glass-border)] shadow-2xl">
    {/* Browser chrome */}
    <div className="bg-[var(--bg-secondary)] px-4 py-3 flex items-center gap-3 border-b border-[var(--glass-border)]">
      <div className="flex gap-1.5">
        <div className="w-3 h-3 rounded-full bg-[#ff5f57]" />
        <div className="w-3 h-3 rounded-full bg-[#ffbd2e]" />
        <div className="w-3 h-3 rounded-full bg-[#28c840]" />
      </div>
      <div className="flex-1 bg-[var(--bg-primary)] rounded-md px-3 py-1.5 flex items-center gap-2 border border-[var(--glass-border)]">
        <div className="w-2.5 h-2.5 rounded-full border border-[var(--text-secondary)]/40" />
        <span className="text-[11px] font-mono text-[var(--text-secondary)] truncate">{url}</span>
      </div>
    </div>
    {children}
  </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-16">
          ...
          <motion.p
            ...
            className="text-[var(--text-secondary)] text-sm font-sans mt-4 max-w-md"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
          ...
          >
            <div>
              <span className="text-[var(--accent-primary)] font-mono text-[10px] font-bold tracking-[0.4em] uppercase opacity-70">FLUTTER · AUTOMOTIVE</span>
              <h3 className="text-5xl font-display font-black tracking-tighter text-[var(--text-primary)] uppercase mt-2">CarDash</h3>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {['Flutter', 'Dart', 'GPS API', 'Google Maps', 'Android'].map(t => (
                <span key={t} className="text-[10px] font-mono font-bold px-3 py-1.5 bg-[var(--bg-secondary)] rounded-full text-[var(--text-secondary)]">{t}</span>
              ))}
            </div>
          </motion.div>
        </div>
        ...
        {DEMOS.map(demo => (
          ...
          >
            <div className="flex items-end justify-between">
              <div>
                <span className="text-[var(--accent-secondary)] font-mono text-[10px] font-bold tracking-[0.4em] uppercase opacity-70">{demo.tag}</span>
                <h3 className="text-5xl font-display font-black tracking-tighter text-[var(--text-primary)] uppercase mt-1">{demo.name}</h3>
                <p className="text-[var(--text-secondary)] text-[13px] mt-2">{demo.description}</p>
              </div>
              <a
                href={demo.url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-5 py-2.5 bg-[var(--text-primary)] text-[var(--bg-primary)] rounded-full text-[10px] font-mono font-bold tracking-widest uppercase hover:bg-[var(--accent-secondary)] transition-colors"
              >
