      ...
      <div className="max-w-3xl mx-auto relative z-10">
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full text-left bg-[var(--card-bg)] border border-[var(--glass-border)] rounded-[20px] px-6 py-5 hover:border-[var(--accent-primary)]/30 transition-all hover:shadow-sm"
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                      {/* Type badge */}
                      <div
                        className="flex items-center gap-1.5 px-3 py-1 rounded-full flex-shrink-0 text-[10px] font-mono font-bold tracking-widest shadow-sm"
                        style={{ 
                      ...
                      >
                        {(() => { const I = Icon as React.FC<React.SVGProps<SVGSVGElement>>; return <I className="w-3 h-3" />; })()}
                        {cfg.label}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-[14px] text-[var(--text-primary)] truncate">{entry.title}</p>
                        <p className="text-[10px] font-mono text-[var(--text-secondary)]/60">{entry.week} · {entry.date}</p>
                      </div>
                    </div>
                    <motion.div
                    ...
                    >
                      <ChevronDown className="w-4 h-4 text-[var(--text-secondary)]/40" />
                    </motion.div>
                  </div>
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="pt-4 mt-4 border-t border-[var(--glass-border)]">
                          <p className="text-[13px] text-[var(--text-secondary)] leading-relaxed mb-4">{entry.description}</p>
                          <div className="flex flex-wrap gap-2">
                            {entry.tags.map(tag => (
                              <span
                                key={tag}
                                className="text-[10px] font-mono font-bold px-3 py-1 rounded-full border border-[var(--glass-border)] bg-[var(--bg-secondary)] text-[var(--text-secondary)]"
                              >
                                {tag}
                              </span>
                            ))}
        ...
        {ENTRIES.length > SHOW_INITIAL && (
          <motion.button
            ...
            className="mt-6 w-full py-4 border border-dashed border-[var(--glass-border)] rounded-[20px] text-[11px] font-mono font-bold tracking-widest uppercase text-[var(--text-secondary)]/60 hover:text-[var(--text-primary)] hover:border-[var(--text-secondary)]/40 transition-all"
          >
