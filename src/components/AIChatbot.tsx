  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-8 right-8 z-[500] w-16 h-16 rounded-full bg-[var(--accent-primary)] text-white flex items-center justify-center shadow-2xl hover:bg-[var(--accent-secondary)] transition-colors group"
      ...
      >
        ...
        <motion.span
          ...
          className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-emerald-400 rounded-full border-2 border-[var(--bg-primary)]"
        />
      </motion.button>
        ...
        {isOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 z-[501] bg-black/40 backdrop-blur-[2px]"
            />
            <motion.div
              ...
              className="fixed bottom-28 right-8 z-[502] w-[380px] max-h-[600px] bg-[var(--card-bg)] rounded-[28px] border border-[var(--glass-border)] shadow-[0_40px_80px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-[var(--accent-primary)] flex items-center justify-center shadow-sm">
                    <Bot className="w-4 h-4 text-white" />
                  </div>
                  <div>
                    <p className="text-[13px] font-bold text-[var(--text-primary)]">Ask about Alins</p>
                    <div className="flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full" />
                      <p className="text-[10px] text-[var(--text-secondary)] font-mono opacity-60">AI-powered · Always on</p>
                    </div>
                  </div>
                </div>
                <button onClick={() => setIsOpen(false)} className="w-8 h-8 rounded-full bg-[var(--text-primary)]/5 flex items-center justify-center hover:bg-[var(--text-primary)]/10 transition-colors">
                  <X className="w-4 h-4 text-[var(--text-secondary)]" />
                </button>
              </div>

              {/* Messages */}
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-0 bg-[var(--bg-primary)]">
                {messages.map((msg, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    <div className={`max-w-[85%] px-4 py-3 rounded-[18px] text-[13px] leading-relaxed shadow-sm ${
                      msg.role === 'user'
                        ? 'bg-[var(--accent-primary)] text-white rounded-br-[6px]'
                        : 'bg-[var(--bg-secondary)] text-[var(--text-primary)] border border-[var(--glass-border)] rounded-bl-[6px]'
                    }`}>
                      {msg.content}
                    </div>
                  </motion.div>
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="bg-[var(--bg-secondary)] border border-[var(--glass-border)] px-4 py-3 rounded-[18px] rounded-bl-[6px] text-[13px] text-[var(--text-secondary)] font-mono">
                      Thinking{dots}
                    </div>
                  </div>
                )}
              ...
              {messages.length === 1 && (
                <div className="px-4 pb-4 bg-[var(--bg-primary)] flex flex-wrap gap-2">
                  {SUGGESTIONS.map(s => (
                    <button
                      key={s}
                      onClick={() => sendMessage(s)}
                      className="text-[11px] px-3 py-1.5 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-full text-[var(--text-secondary)] hover:bg-[var(--accent-primary)] hover:text-white hover:border-[var(--accent-primary)] transition-all font-medium"
                    >
                      {s}
                    </button>
                  ))}
              ...
              {/* Input */}
              <div className="px-4 py-4 border-t border-[var(--glass-border)] bg-[var(--bg-secondary)] flex items-center gap-3">
                <input
                  ref={inputRef}
                  value={input}
                  ...
                  className="flex-1 bg-[var(--bg-primary)] rounded-full px-4 py-2.5 text-[13px] text-[var(--text-primary)] outline-none border border-[var(--glass-border)] focus:border-[var(--accent-primary)] transition-all"
                />
                <button
                  onClick={() => sendMessage(input)}
                  disabled={!input.trim() || isLoading}
                  className="w-9 h-9 rounded-full bg-[var(--accent-primary)] flex items-center justify-center disabled:opacity-30 hover:bg-[var(--accent-secondary)] transition-all shadow-md"
                >
