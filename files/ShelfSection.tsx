...
export const ShelfSection = () => {
  ...
  return (
    <section className="relative bg-transparent px-8 md:px-20 py-32 border-t border-black/[0.06] overflow-hidden">
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-amber-50/40 blur-[120px] pointer-events-none rounded-full" />
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          {/* Tab switcher */}
          <div className="flex items-center gap-2 p-1.5 bg-[#f1f5f9] rounded-full">
            {([['books', BookOpen], ['music', Music2]] as const).map(([tab, Icon]) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-[11px] font-mono font-bold uppercase tracking-widest transition-all ${
                  activeTab === tab ? 'bg-white text-[#0a0a0a] shadow-sm' : 'text-[#64748b]'
                }`}
              >
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'books' ? (
            ...
            >
              {/* Shelf */}
              <div className="bg-[#fafaf8] rounded-[32px] border border-black/[0.06] p-8 pb-0 shadow-inner overflow-hidden">
                ...
                {/* Shelf plank */}
                <div className="h-4 bg-gradient-to-b from-[#d4a96a] to-[#b8873e] rounded-b-[24px] shadow-[0_4px_12px_rgba(0,0,0,0.1)]" />
              </div>
              ...
              <AnimatePresence>
                {selectedBook && (
                  ...
                  >
                    <div className="bg-white border border-black/[0.06] rounded-[20px] p-6 flex items-start gap-4">
                      <div
                        className="w-12 h-16 rounded-sm flex items-center justify-center flex-shrink-0 text-xl"
                        style={{ background: selectedBook.color }}
                      >
                        {selectedBook.emoji}
                      </div>
                      <div>
                        <p className="font-bold text-[15px] text-[#0a0a0a]">{selectedBook.title}</p>
                        <p className="text-[12px] text-[#64748b] font-mono mb-2">by {selectedBook.author}</p>
                        <p className="text-[13px] text-[#374151] italic">"{selectedBook.note}"</p>
                      </div>
                    </div>
                  </motion.div>
                )}
              ...
              {MUSIC.map((track, i) => (
                <motion.button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-[16px] border text-left transition-all ${
                    selectedTrack?.title === track.title
                      ? 'bg-white border-[#0369a1]/30 shadow-lg'
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-[14px] text-[#0a0a0a] truncate">{track.title}</p>
                    <p className="text-[11px] text-[#64748b] font-mono">{track.artist} · {track.genre}</p>
                    <AnimatePresence>
                      {selectedTrack?.title === track.title && (
                        <motion.p
                          ...
                          className="text-[11px] text-[#0369a1] italic mt-1"
                        >
                          {track.note}
                        </motion.p>
                      )}
                  ...
                  <motion.div
                    ...
                    className={`w-2 h-2 rounded-full flex-shrink-0 ml-auto ${selectedTrack?.title === track.title ? 'bg-emerald-400' : 'bg-[#e2e8f0]'}`}
                  />
                </motion.button>
              ))}
            </motion.div>
